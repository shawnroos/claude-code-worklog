import { WorkItem, WorkItemReference, SimilarityScore } from '../types'
...
export class SmartReferenceEngine {
   ...
   */
  public generateAutomaticReferences(activeItem: WorkItem): SmartReference[] {
    ...
    for (const historicalItem of historicalItems) {
      ...
      if (similarity.total_score >= this.similarityThreshold) {
        const reference: SmartReference = {
          id: this.generateId(),
   ...
   */
  public calculateSemanticSimilarity(item1: WorkItem, item2: WorkItem): SimilarityScore {
    ...
    const locationOverlap = locations1.filter((l: string) => locations2.includes(l))
   ...
   */
  public getContextualSuggestions(activeItems: WorkItem[]): ContextualSuggestion[] {
   ...
   */
  public updateReferencesOnChange(itemId: string): void {
  ...
  private getRelevantHistoricalItems(activeItem: WorkItem): WorkItem[] {
    ...
    const uniqueItems = historicalItems.filter((item, index, self) => 
  ...
  ): SmartReference['relationship_type'] {
  ...
  private updateItemReferences(item: WorkItem, references: SmartReference[]): void {
